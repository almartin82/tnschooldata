# ==============================================================================
# LIVE Pipeline Tests for tnschooldata
# ==============================================================================
#
...
test_that("Tennessee DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Tennessee DOE URL and verify HTTP 200")
})
...
test_that("Can download Tennessee enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Tennessee enrollment file", {
  skip_if_offline()
...
test_that("tnschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- tnschooldata::get_available_years()
  ...
  tryCatch({
    raw <- tnschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- tnschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- tnschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- tnschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- tnschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- tnschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- tnschooldata:::get_cache_path(2024, "enrollment")
