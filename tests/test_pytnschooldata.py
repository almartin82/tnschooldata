    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_state' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # Tennessee should have ~1 million students (800K-1.2M)
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        """Returns a pandas DataFrame."""
        import pytnschooldata as tn
        df = tn.fetch_enr_multi([2023, 2024])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_more_rows_than_single_year(self):
        ...
        import pytnschooldata as tn
        df_single = tn.fetch_enr(2024)
        df_multi = tn.fetch_enr_multi([2023, 2024])
...
class TestDataIntegrity:
    ...
    def test_consistent_between_single_and_multi(self):
        ...
        import pytnschooldata as tn
        df_single = tn.fetch_enr(2024)
        df_multi = tn.fetch_enr_multi([2024])
