"""
Tests for pytnschooldata Python wrapper.
...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = tn.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pytnschooldata as tn
        assert hasattr(tn, 'fetch_enr')
        assert hasattr(tn, 'fetch_enr_multi')
        assert hasattr(tn, 'tidy_enr')
        assert hasattr(tn, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pytnschooldata as tn
        assert hasattr(tn, '__version__')
        assert isinstance(tn.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = tn.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = tn.fetch_enr(years['max_year'])
        # Total enrollment in Tennessee varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Tennessee should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = tn.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = tn.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pytnschooldata as tn
        years = get_test_years()
        df_single = tn.fetch_enr(years['max_year'])
        df_multi = tn.fetch_enr_multi([years['max_year']])
