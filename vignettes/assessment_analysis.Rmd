---
title: "Tennessee Assessment Data Analysis"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Tennessee Assessment Data Analysis}
...
Tennessee's TCAP (Tennessee Comprehensive Assessment Program) tests students in grades 3-8 on ELA, Math, Science, and Social Studies. High school students take End-of-Course (EOC) exams. Results are reported across four proficiency levels:
...
Tennessee students showed steady improvement in proficiency rates across all subjects in 2024.

```{r story1-state-proficiency}
assess_2024 |>
...
```{r story1-plot, fig.cap="State proficiency rates by subject"}
assess_2024 |>
  filter(is_state, subgroup == "All Students", grade == "All" | grade == "ALL") |>
  filter(proficiency_level %in% c("on_track", "mastered")) |>
  group_by(subject) |>
...
Math proficiency lags behind ELA across Tennessee, a pattern consistent with national trends.
...
```{r story2-plot, fig.cap="Math vs ELA proficiency by grade"}
assess_2024 |>
  filter(is_state, subgroup == "All Students") |>
  filter(subject %in% c("Math", "ELA")) |>
  filter(grade %in% c("03", "04", "05", "06", "07", "08")) |>
...
Third grade reading proficiency is a critical milestone - students who can't read by grade 3 are four times more likely to drop out of high school.

```{r story3-grade3-reading}
assess_2024 |>
...
```{r story3-plot, fig.cap="Grade 3 ELA proficiency distribution"}
assess_2024 |>
...
Significant proficiency gaps persist between racial/ethnic groups in Tennessee.
...
```{r story4-plot, fig.cap="Math proficiency by race/ethnicity"}
assess_2024 |>
  filter(is_state, grade == "All" | grade == "ALL", subject == "Math") |>
  filter(subgroup %in% race_groups) |>
  filter(proficiency_level %in% c("on_track", "mastered")) |>
...
Students from economically disadvantaged backgrounds score 20+ percentage points lower than their peers.
...
```{r story5-plot, fig.cap="Achievement gap by economic status"}
assess_2024 |>
  filter(is_state, grade == "All" | grade == "ALL", subject == "Math") |>
  filter(subgroup %in% c("Economically Disadvantaged", "Non-Economically Disadvantaged")) |>
  filter(proficiency_level %in% c("on_track", "mastered")) |>
...
English Learners face significant challenges on Tennessee assessments, especially in ELA.

```{r story6-el-performance}
assess_2024 |>
  filter(is_state, grade == "All" | grade == "ALL") |>
...
```{r story6-plot, fig.cap="English Learner performance vs state average"}
assess_2024 |>
  filter(is_state, grade == "All" | grade == "ALL") |>
...
Students with disabilities have the lowest proficiency rates of any subgroup.
...
```{r story7-plot, fig.cap="Students with Disabilities ELA proficiency"}
assess_2024 |>
  filter(is_state, grade == "All" | grade == "ALL") |>
...
Female students outperform male students in ELA, while male students have a slight edge in math.
...
```{r story8-plot, fig.cap="Gender gaps across subjects"}
assess_2024 |>
  filter(is_state, grade == "All" | grade == "ALL") |>
  filter(subject %in% c("Math", "ELA", "Science")) |>
  filter(subgroup %in% c("Male", "Female")) |>
...
```{r story9-plot, fig.cap="Math proficiency declines through middle school"}
assess_2024 |>
  filter(is_state, subgroup == "All Students", subject == "Math") |>
  filter(grade %in% c("03", "04", "05", "06", "07", "08")) |>
  filter(proficiency_level %in% c("on_track", "mastered")) |>
...
Science assessment results show opportunities for improvement in STEM education.
...
```{r story10-plot, fig.cap="Science proficiency by grade"}
assess_2024 |>
  filter(is_state, subgroup == "All Students", subject == "Science") |>
  filter(grade %in% c("03", "04", "05", "06", "07", "08")) |>
  filter(proficiency_level %in% c("on_track", "mastered")) |>
...
Understanding the full distribution of student performance reveals more than proficiency rates alone.

```{r story11-distribution}
assess_2024 |>
  filter(is_state, subgroup == "All Students", subject == "Math") |>
...
```{r story11-plot, fig.cap="Full proficiency distribution for Math"}
assess_2024 |>
  filter(is_state, subgroup == "All Students", subject == "Math") |>
  filter(grade == "All" | grade == "ALL") |>
  mutate(proficiency_level = factor(
...
```{r story12-plot, fig.cap="EOC proficiency rates"}
assess_2024 |>
  filter(is_state, subgroup == "All Students") |>
  filter(subject %in% eoc_subjects) |>
  filter(proficiency_level %in% c("on_track", "mastered")) |>
...
```{r story13-plot, eval=FALSE, fig.cap="Year-over-year proficiency trends"}
assess_multi |>
  filter(is_state, subgroup == "All Students") |>
  filter(subject %in% c("Math", "ELA")) |>
  filter(grade == "All" | grade == "ALL") |>
...
Comparing 2019 (pre-COVID) to 2024 shows pandemic learning recovery progress.

```{r story14-recovery, eval=FALSE}
...
```{r story14-plot, eval=FALSE, fig.cap="COVID learning recovery"}
assess_recovery |>
  filter(is_state, subgroup == "All Students", subject == "Math") |>
  filter(grade == "All" | grade == "ALL") |>
  filter(proficiency_level %in% c("on_track", "mastered")) |>
...
Identifying which groups achieve mastery (not just proficiency) at the highest rates.
...
```{r story15-plot, fig.cap="Mastery rates by subgroup"}
assess_2024 |>
  filter(is_state, subject == "Math") |>
  filter(grade == "All" | grade == "ALL") |>
  filter(proficiency_level == "mastered") |>
...
### Data Source
Assessment data is sourced directly from the Tennessee Department of Education accountability portal. Files are downloaded from:
